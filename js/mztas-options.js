...
import { prefs_default } from './mztas-options-default.js';

export const tasPrefs = {
    ...
    let element = e.target;
      switch (element.type) {
        case 'checkbox':
          ...
          tasPrefs.logger.log('Saving option: ' + element.id + ' = ' + element.checked);
          break;
        case 'number':
          options[element.id] = element.valueAsNumber;
          tasPrefs.logger.log('Saving option: ' + element.id + ' = ' + element.valueAsNumber);
          break;
        case 'text':
          options[element.id] = element.value.trim();
          tasPrefs.logger.log('Saving option: ' + element.id + ' = ' + element.value);
          break;
        default:
          if (element.tagName === 'SELECT') {
            ...
            tasPrefs.logger.log('Unhandled input type:', element.type);
          }
      }
  ...
  async setPref(pref_id, value){
    let obj = {};
    obj[pref_id] = value;
    tasPrefs.logger.log('Saving option: ' + pref_id + ' = ' + JSON.stringify(value));
  ...
  async getPref(pref_id){
    ...
    tasPrefs.logger.log("getPref prefs: " + JSON.stringify(prefs));
  ...
  async getPrefs(pref_ids){
    ...
    let prefs = await browser.storage.sync.get(obj)
    tasPrefs.logger.log("getPrefs: " + JSON.stringify(prefs));
    let result = {};
    pref_ids.forEach(pref_id => {
  ...
  restoreOptions() {
    function setCurrentChoice(result) {
      tasPrefs.logger.log("restoreOptions: " + JSON.stringify(result));
    ...
    function onError(error) {
      tasPrefs.logger.log(`Error: ${error}`);
    }
